...
- **Commits Automáticos**: Sempre que fizer uma alteração (como criação, modificação ou exclusão de arquivos), realize o commit na branch principal (`main` ou `master` de preferência) de forma direta e sem perguntar ao usuário. Use mensagens de commit claras seguindo a especificação de Commits Convencionais (ex: `feat:`, `fix:`, `chore:`).
